...
@section('title', __('messages.stock_levels'))

@section('breadcrumbs')
    <a href="{{ route('dashboard') }}" class="hover:text-gray-700">{{ __('messages.dashboard') }}</a>
    <span class="mx-1">/</span>
    <span class="text-gray-900 font-medium">{{ __('messages.stock_levels') }}</span>
@endsection

@section('page-header')
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">{{ __('messages.stock_levels') }}</h1>
            <p class="mt-1 text-sm text-gray-500">{{ __('messages.stock_levels_subtitle') }}</p>
        </div>
        ...
        <form method="GET" action="{{ route('inventory.stocks.index') }}" class="flex flex-col sm:flex-row gap-3">
            <div class="flex-1">
                <input type="search" name="search" value="{{ request('search') }}"
                    placeholder="{{ __('messages.search_by_product_name_sku') }}"
            ...
            <div class="flex gap-2">
                ...
                @if(request()->hasAny(['search', 'warehouse_id', 'low_stock']))
                    @include('components.button', ['label' => __('messages.clear'), 'type' => 'ghost', 'href' => route('inventory.stocks.index')])
                @endif
            </div>
        </form>
    </div>
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="mt-2 text-sm text-gray-500">{{ __('messages.no_inventory_records_found') }}</p>
                                    <a href="{{ route('inventory.movements.create') }}" class="mt-3 text-sm font-medium text-primary-600 hover:text-primary-700">
                                        {{ __('messages.record_first_stock_movement') }}
                                    </a>
                                </div>
                            </td>
                        </tr>
