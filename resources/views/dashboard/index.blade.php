...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-gradient-to-br from-blue-600 to-blue-700 rounded-2xl p-6 text-white shadow-lg">
            <div class="flex justify-between items-start mb-4">
                <div>
                    ...
                    <p class="text-3xl font-bold mt-1">{{ format_currency($salesStats['total_revenue']) }}</p>
                </div>
        ...
        <div class="bg-white rounded-2xl p-6 shadow-sm ring-1 ring-gray-100">
            ...
            <p class="text-2xl font-bold text-gray-900 mb-3">{{ format_currency($arStats['total_outstanding']) }}</p>
            <div class="space-y-2">
        ...
        <div class="bg-white rounded-2xl p-6 shadow-sm ring-1 ring-gray-100">
            ...
            <p class="text-2xl font-bold text-gray-900 mb-3">{{ format_currency($apStats['total_outstanding']) }}</p>
            <div class="space-y-2">
    ...
    {{-- Row 3: Revenue/Expense Chart + Quick Actions --}}
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {{-- Revenue & Expense Chart --}}
        <div class="lg:col-span-2 bg-white rounded-2xl p-6 shadow-sm ring-1 ring-gray-100">
            <div class="flex items-center justify-between mb-6">
                <h3 class="font-semibold text-gray-900">{{ __('dashboard.revenue_vs_expense') }}</h3>
                <div class="flex items-center gap-4 text-xs">
                    ...
                    <span class="flex items-center gap-1.5"><span class="h-2.5 w-2.5 rounded-full bg-red-400"></span> {{ __('dashboard.expense') }}</span>
                </div>
            </div>
            <div class="h-56" x-data="revenueChart()" x-init="init()">
...
function revenueChart() {
    return {
        init() {
            ...
            const labels = @json(collect($revenueChart)->keys()->map(fn($m) => \Carbon\Carbon::createFromFormat('Y-m', $m)->translatedFormat('M Y'))->values());
            const revenueData = @json(array_values($revenueChart));
            const expenseData = @json(array_values($expenseChart));
