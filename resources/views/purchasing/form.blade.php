    ...
    <form method="POST"
        ...
        <div class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-gray-100">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div>
                    <label for="payment_terms" class="block text-sm font-medium text-gray-700 mb-1">{{ __('messages.convert_payment_terms') }}</label>
                    <select name="payment_terms" id="payment_terms"
                        class="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-2.5 text-sm text-gray-700 focus:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500">
                        @foreach(\App\Models\PurchaseOrder::paymentTermsOptions() as $pt)
                            <option value="{{ $pt }}" @selected(old('payment_terms', $order->payment_terms ?? 'net_30') === $pt)>{{ __('messages.payment_term_' . $pt) }}</option>
                        @endforeach
                    </select>
            ...
            <div class="mt-4">
                ...
                <textarea name="shipping_address" id="shipping_address" rows="2"
                    ...
                    placeholder="{{ __('messages.convert_shipping_placeholder') }}">{{ old('shipping_address', $order->shipping_address) }}</textarea>
