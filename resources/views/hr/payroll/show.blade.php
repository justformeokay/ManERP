...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-gray-100">
            ...
            <p class="mt-2 text-xl font-bold text-gray-900">Rp {{ number_format($summary['total_gross'], 0, ',', '.') }}</p>
        </div>
        <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-gray-100">
            <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ __('messages.total_net') }}</p>
            <p class="mt-2 text-xl font-bold text-green-700">Rp {{ number_format($summary['total_net'], 0, ',', '.') }}</p>
        </div>
        <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-gray-100">
            <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider">PPh 21</p>
            <p class="mt-2 text-xl font-bold text-amber-700">Rp {{ number_format($summary['total_pph21'], 0, ',', '.') }}</p>
        </div>
        <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-gray-100">
            <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider">BPJS ({{ __('messages.company') }})</p>
            <p class="mt-2 text-xl font-bold text-blue-700">Rp {{ number_format($summary['total_bpjs_co'], 0, ',', '.') }}</p>
        </div>
    </div>
            ...
            <table class="min-w-full divide-y divide-gray-100">
                ...
                <tbody class="divide-y divide-gray-50">
                    @foreach($period->payslips->sortBy('employee.name') as $slip)
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-sm text-right text-gray-900">{{ number_format($slip->gross_salary, 0, ',', '.') }}</td>
                            <td class="px-6 py-4 text-sm text-right text-red-600">{{ number_format($slip->total_bpjs_employee, 0, ',', '.') }}</td>
                            <td class="px-6 py-4 text-sm text-right text-amber-700">{{ number_format($slip->pph21_amount, 0, ',', '.') }}</td>
                            <td class="px-6 py-4 text-sm text-right text-gray-600">{{ number_format((float)$slip->loan_deduction + (float)$slip->absence_deduction + (float)$slip->other_deductions, 0, ',', '.') }}</td>
                            <td class="px-6 py-4 text-sm text-right font-semibold text-gray-900">{{ number_format($slip->net_salary, 0, ',', '.') }}</td>
                ...
                <tfoot class="bg-gray-50/50">
                    <tr class="font-semibold">
                        ...
                        <td class="px-6 py-3 text-sm text-right text-gray-900">{{ number_format($summary['total_gross'], 0, ',', '.') }}</td>
                        <td class="px-6 py-3 text-sm text-right text-red-600">{{ number_format($summary['total_bpjs_emp'], 0, ',', '.') }}</td>
                        <td class="px-6 py-3 text-sm text-right text-amber-700">{{ number_format($summary['total_pph21'], 0, ',', '.') }}</td>
                        <td class="px-6 py-3 text-sm text-right text-gray-600">—</td>
                        <td class="px-6 py-3 text-sm text-right text-gray-900">{{ number_format($summary['total_net'], 0, ',', '.') }}</td>
