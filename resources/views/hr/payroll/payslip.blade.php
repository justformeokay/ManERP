...
<div class="max-w-3xl mx-auto space-y-6">
            ...
            <tbody class="divide-y divide-gray-50">
                @foreach($payslip->items->where('type', 'earning') as $item)
                    <tr>
                        ...
                        <td class="py-2 text-right font-medium text-gray-900">Rp {{ number_format($item->amount, 0, ',', '.') }}</td>
                    </tr>
                @endforeach
            </tbody>
            <tfoot>
                <tr class="border-t-2 border-gray-200 font-semibold">
                    ...
                    <td class="py-3 text-right text-gray-900">Rp {{ number_format($payslip->gross_salary, 0, ',', '.') }}</td>
                </tr>
            </tfoot>
        </table>
    </div>
        ...
        <table class="w-full text-sm">
            <tbody class="divide-y divide-gray-50">
                @foreach($payslip->items->where('type', 'deduction') as $item)
                    <tr>
                        <td class="py-2 text-gray-700">{{ $item->label }}</td>
                        <td class="py-2 text-right font-medium text-red-600">Rp {{ number_format($item->amount, 0, ',', '.') }}</td>
                    </tr>
                @endforeach
            </tbody>
            <tfoot>
                <tr class="border-t-2 border-gray-200 font-semibold">
                    ...
                    <td class="py-3 text-right text-red-700">Rp {{ number_format($payslip->total_deductions, 0, ',', '.') }}</td>
                </tr>
            </tfoot>
        </table>
    </div>
    ...
    <div class="rounded-2xl bg-blue-50 p-6 shadow-sm ring-1 ring-blue-100">
        ...
        <div class="grid grid-cols-2 md:grid-cols-5 gap-3 text-sm">
            <div><span class="text-blue-600">JHT 3.7%</span><p class="font-medium text-blue-900">{{ number_format($payslip->bpjs_jht_company, 0, ',', '.') }}</p></div>
            <div><span class="text-blue-600">JKK</span><p class="font-medium text-blue-900">{{ number_format($payslip->bpjs_jkk_company, 0, ',', '.') }}</p></div>
            <div><span class="text-blue-600">JKM</span><p class="font-medium text-blue-900">{{ number_format($payslip->bpjs_jkm_company, 0, ',', '.') }}</p></div>
            <div><span class="text-blue-600">JP 2%</span><p class="font-medium text-blue-900">{{ number_format($payslip->bpjs_jp_company, 0, ',', '.') }}</p></div>
            <div><span class="text-blue-600">Kes 4%</span><p class="font-medium text-blue-900">{{ number_format($payslip->bpjs_kes_company, 0, ',', '.') }}</p></div>
        </div>
        <p class="mt-3 text-sm font-semibold text-blue-900">Total: Rp {{ number_format($payslip->total_bpjs_company, 0, ',', '.') }}</p>
    </div>
    ...
    <div class="rounded-2xl bg-green-50 p-6 shadow-sm ring-1 ring-green-100">
        <div class="flex items-center justify-between">
            ...
            <p class="text-2xl font-bold text-green-800">Rp {{ number_format($payslip->net_salary, 0, ',', '.') }}</p>
        </div>
    </div>
</div>
@endsection
