...
@section('breadcrumbs')
    ...
    <span class="text-gray-900 font-medium">{{ __('messages.warehouses_title') }}</span>
@endsection

@section('page-header')
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">{{ __('messages.warehouses_title') }}</h1>
            <p class="mt-1 text-sm text-gray-500">{{ __('messages.manage_warehouses') }}</p>
        </div>
        @include('components.button', [
            'label' => __('messages.add_warehouse'),
            'type' => 'primary',
        ...
        <form method="GET" action="{{ route('warehouses.index') }}" class="flex flex-col sm:flex-row gap-3">
            <div class="flex-1">
                <input type="search" name="search" value="{{ request('search') }}"
                    placeholder="{{ __('messages.search_warehouse_placeholder') }}"
            ...
            <div class="flex gap-2">
                ...
                @if(request()->hasAny(['search']))
                    @include('components.button', ['label' => __('messages.clear_btn'), 'type' => 'ghost', 'href' => route('warehouses.index')])
                @endif
            </div>
        </form>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-100">
                <thead class="bg-gray-50/50">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">{{ __('messages.warehouse_code_header') }}</th>
                        <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">{{ __('messages.warehouse_name_header') }}</th>
                        <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">{{ __('messages.warehouse_address_header') }}</th>
                        <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">{{ __('messages.warehouse_default_header') }}</th>
                        <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">{{ __('messages.warehouse_status_header') }}</th>
                        <th class="px-6 py-3 text-right text-xs font-semibold uppercase tracking-wider text-gray-500">{{ __('messages.warehouse_actions_header') }}</th>
                    </tr>
                </thead>
                        ...
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                @if($warehouse->is_default)
                                    <span class="inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium bg-blue-50 text-blue-700 ring-1 ring-blue-600/20">{{ __('messages.default_label') }}</span>
                                @else
                            ...
                            <td class="whitespace-nowrap px-6 py-4">
                                <span class="inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ring-1
                                    ...
                                    {{ $warehouse->is_active ? __('messages.active') : __('messages.inactive') }}
                                </span>
                            </td>
                            <td class="whitespace-nowrap px-6 py-4 text-right text-sm space-x-1">
                                ...
                                <form method="POST" action="{{ route('warehouses.destroy', $warehouse) }}" class="inline"
                                      onsubmit="return confirm('{{ __('messages.delete_warehouse_confirm') }} {{ $warehouse->name }}?')">
                                    @csrf @method('DELETE')
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="mt-2 text-sm text-gray-500">{{ __('messages.no_warehouses_found') }}</p>
                                    <a href="{{ route('warehouses.create') }}" class="mt-3 text-sm font-medium text-blue-600 hover:text-blue-700">
                                        {{ __('messages.add_your_first_warehouse') }}
                                    </a>
                                </div>
                            </td>
                        </tr>
